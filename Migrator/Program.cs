...
namespace Migrator
{
    ...
    class Program
    {
        ...
        static string mongoConnectionString = "mongodb+srv://carlos:d-2611983@cluster0.mosfx.mongodb.net/<dbname>?retryWrites=true&w=majority";
        ...
        static async Task Main(string[] args)
        {
            ...
            if (datePipelineResults.Count > 0)
            {
                ...
                {
                    list.Add(new ReplaceOneModel<Movie>(new FilterDefinitionBuilder<Movie>().Where(m => m.Id == movie.Id), movie));
                }
            ...
            if (ratingPipelineResults.Count > 0)
            {
                ...
                var list2 = new List<ReplaceOneModel<Movie>>();

                foreach (var movie in ratingPipelineResults)
                {
                    list2.Add(new ReplaceOneModel<Movie>(new FilterDefinitionBuilder<Movie>().Where(m => m.Id == movie.Id), movie));
                }

                bulkWriteRatingsResult = await _moviesCollection.BulkWriteAsync(list2);
