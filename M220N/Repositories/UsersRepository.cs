    ...
    public class UsersRepository
    {
        ...
        */
        ...
        public async Task<User> GetUserAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            return await _usersCollection.Find<User>(filter).FirstOrDefaultAsync();
        }
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var filter = new BsonDocument("user_id", user.Email);
                var updates = Builders<Session>.Update.Set(s => s.UserId, user.Email).Set(s => s.Jwt, user.AuthToken);
                var options = new UpdateOptions { IsUpsert = true };
                await _sessionsCollection.UpdateOneAsync(filter, updates, options);
            ...
            Dictionary<string, string> preferences, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                */
                ...
                var updates = Builders<User>.Update.Set(u => u.Preferences, preferences);
                var options = new UpdateOptions { IsUpsert = false };
                updateResult = await _usersCollection.UpdateOneAsync(filter, updates, options);
